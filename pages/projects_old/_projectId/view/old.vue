...
<div class="body flex-container">
  ...
  <page-content v-model="article_draft.data" v-if="article_draft.data" 
  ...
  methods: {
    ...
    loadProj() {
      ...
      this.$store.dispatch("articles/read_article", { _id: this.article_id }).then(() => {
        let _article = this.$store.getters['articles/article_query']({ _id: this.article_id})[0];
        console.warn(_article);
        this.article_draft.title = _article.title;
        this.article_draft.description = _article.description;
        this.article_draft.data = _article.data;
        this.article_draft.id = _article._id
      })
